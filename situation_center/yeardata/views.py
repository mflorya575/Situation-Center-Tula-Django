...
def industrialprod_detail(request, slug):
    ...
    try:
        ...
        if df_melted.empty:
            ...
            fig_linear.add_trace(go.Bar(x=df_melted['year'], y=df_melted['data'], name='Столбчатая диаграмма'))
            fig_linear.update_layout(
                title=f'{industrialprod.title} - Комбинированный график (Линейная шкала)',
                xaxis_title='Годы',
                yaxis_title='Количество'
            )
            combined_chart_linear = fig_linear.to_html(full_html=False)
            ...
            fig_log.add_trace(go.Bar(x=df_melted['year'], y=df_melted['data'], name='Столбчатая диаграмма'))
            fig_log.update_layout(
                title=f'{industrialprod.title} - Комбинированный график (Логарифмическая шкала)',
                xaxis_title='Годы',
                yaxis_title='Количество',
                yaxis_type='log'
            )
            ...
            map_fig_linear = px.choropleth(df_latest,
                                           ...
                                           color='data',
                                           hover_name='region',
                                           title=f'{industrialprod.title} - {latest_year} (Линейная шкала)',
                                           color_continuous_scale='Reds')
            map_fig_linear.update_geos(fitbounds="locations", visible=False)
            map_chart_linear = map_fig_linear.to_html(full_html=False)
            ...
            map_fig_log = px.choropleth(df_latest,
                                        ...
                                        color='data',
                                        hover_name='region',
                                        title=f'{industrialprod.title} - {latest_year} (Логарифмическая шкала)',
                                        color_continuous_scale='Reds',
                                        range_color=[df_latest['data'].min(), df_latest['data'].max()],
                                        color_continuous_midpoint=0.1)
            map_fig_log.update_geos(fitbounds="locations", visible=False)
    ...
    return render(request, 'yeardata/industrialprod_detail.html', context)
...
def building_detail(request, slug):
    ...
    return render(request, 'yeardata/building_detail.html', context)
...
def transport_detail(request, slug):
    ...
    return render(request, 'yeardata/transport_detail.html', context)
...
def science_detail(request, slug):
    ...
    return render(request, 'yeardata/science_detail.html', context)
...
def price_detail(request, slug):
    ...
    return render(request, 'yeardata/price_detail.html', context)
